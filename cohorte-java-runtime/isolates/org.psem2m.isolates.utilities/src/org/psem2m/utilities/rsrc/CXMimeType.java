...
public class CXMimeType {
	...
	public static enum EXType {
		APPLICATION("application"), AUDIO("audio"), FONT("font"), IMAGE("image"), MESSAGE("message"), MODEL(
				"model"), MULTIPART("multipart"), PLUGIN("plugin"), TEXT("text"), VIDEO("video"), XWORLD(
				"x-world");
	...
	private static final String ADDON_DEF_EXT = "_def";
	private static final String ADDON_IS_TEXT = "_text";

	private static final String MESS_ERR_CCL = "Can't get caller of package [%s]. \nException:\n%s";

	private static HashMap<String, CXMimeType> sExtToMime = new HashMap<String, CXMimeType>(250);
	private static HashMap<String, CXMimeType> sMimeTypes = new HashMap<String, CXMimeType>(200);
	...
	static {
		try {
			int wPos = CXMimeType.class.getName().indexOf(CXMimeType.class.getSimpleName());
			if (wPos != -1 && wPos != 0) {
				...
				ResourceBundle wBndl = ResourceBundle.getBundle(wResourceId, Locale.getDefault(),
						wClassLoader);
				...
				for (Iterator<String> xIt = wBndl.keySet().iterator(); xIt.hasNext();) {
					...
					if (wMimeType == null) {
						...
						if (wMimeType.isValid()) {
							...
							log("MimeType [%s] - Ext[%s] is not valid", wMime, wExt);
						}
					} else {
	...
	protected static ClassLoader getCallerClassLoader(Class<?> aClass) {
		...
		try {
			...
			String wMess = String.format(MESS_ERR_CCL, wPackageName, CXException.eInString(e));
			log(wMess);
			return null;
		}
	}
	...
	public static String getMimeTypeDescr() {
		...
		CXMimeType[] wArray = sMimeTypes.values().toArray(new CXMimeType[sMimeTypes.size()]);
	...
	private static void log(String aFormat, Object... aArgs) {
		...
		System.out.println(String.format("+++ org.psem2m.utilities.rsrc.CXMimeType %s",
				wLine.replace('\n', '§')));
	}
	...
	private boolean addExt(String aExt, String aMimeType) {
		...
		if (aExt != null && !aExt.isEmpty() && aMimeType != null && !aMimeType.isEmpty()) {
			...
			if (wPos != -1) {
				...
				if (wAddOn.indexOf(ADDON_DEF_EXT) >= 0) {
					if (pDefExt != null && sTest) {
						System.out.println("Mime has already default extension [" + aExt
								+ "] Mime[" + aMimeType + "]");
					}
					pDefExt = aExt;
				}
	...
	/**
	 * @return Full mime type : type/subtype
	 */
	...
	public String toStringDescr() {
		...
		StringBuilder wSB = new StringBuilder().append(pMimeType).append(" - isText[")
				.append(pIsText).append("] - defExt[").append(getDefaultExt()).append("]");
		...
		if (sTest && !pIsText && (pMimeType.indexOf("htm") != -1 || pMimeType.indexOf("xml") != -1)) {
