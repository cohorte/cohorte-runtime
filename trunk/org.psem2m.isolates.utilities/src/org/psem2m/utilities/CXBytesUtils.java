...
public class CXBytesUtils
{
   ...
   */
  /*//LangCS
   private final static String DEF_ISO_8859_1_CONSTANT_JSHARP = DEF_ISO_8859_1_CONSTANT_JAVA;     
   //FLangCS  */

  /**
   * java.nio.charset.Charset: Seven-bit ASCII, a.k.a. ISO646-US, a.k.a. the Basic Latin block of
   * the Unicode character set
   */
  private final static String DEF_US_ASCII_CONSTANT_JAVA   = "US-ASCII";

  /**
   * .NET: Encodes Unicode characters as single, 7-bit ASCII characters.
   * 
  ...
  /**
   * java.nio.charset.Charset: Sixteen-bit UCS Transformation Format, byte order identified by an
   * optional byte-order mark
   */
  private final static String DEF_UTF_16_CONSTANT_JAVA     = "UTF-16";

  /**
   * .NET : Encodes each Unicode character as two consecutive bytes, using little endian (code page
   * 1200) byte ordering.
   */
  ...
  /**
   * java.nio.charset.Charset: Sixteen-bit UCS Transformation Format, big-endian byte order
   */
  private final static String DEF_UTF_16BE_CONSTANT_JAVA   = "UTF-16BE";

  /**
   * Encodes each Unicode character as two consecutive bytes, using big endian (code page 1201) byte
   * ordering.
   */
  ...
  /**
   * java.nio.charset.Charset: Sixteen-bit UCS Transformation Format, little-endian byte order
   */
  ...
  /**
   * .NET : Encodes Unicode characters using the UTF-7 encoding. (UTF-7 stands for UCS
   * Transformation Format, 7-bit form.) This encoding supports all Unicode character values and can
   * be accessed as code page 65000.
   */
  ...
  /**
   * .NET : Encodes Unicode characters using the UTF-8 encoding. (UTF-8 stands for UCS
   * Transformation Format, 8-bit form.) This encoding supports all Unicode character values and can
   * be accessed as code page 65001.
   */
   ...
   * @return
   */
  static public StringBuilder addByte2DecimalInSB(StringBuilder aSB, byte mybyte)
  {
    aSB.append('.');
    aSB.append('d');
   ...
   * @return
   */
  static public StringBuilder addByte2HexaInCSB(StringBuilder aSB, byte mybyte)
  {
    aSB.append('.');
    aSB.append('x');
   ...
   * @return
   */
  static public StringBuilder addByte2HexaInSB(StringBuilder aSB, byte mybyte)
  {
    aSB.append('.');
    aSB.append('x');
    aSB.append(HEXA_DIGITS[(0x00F0 & mybyte) >> 4]);
  ...
  /**
   * Ajoute aAddedLen octets du aAddedBuffer a partir de aAddedOffset dans le buffer "extensible"
   * aDestBuffer en position aDestPos
   * 
   * @param aDestBuffer
   * @param aDestPos
   * @param aAddedBuffer
   ...
   * @return
   */
  private static boolean checkBOM(byte[] aBuffer, byte[] aBomRef)
  {
   ...
   * @return
   */
  public static boolean contains(byte[] data, int aOffset, int aLen, byte[] pattern)
  {
   ...
   * @return
   */
  public static int getBomLength(byte[] aBuffer)
  {
   ...
   * @return
   */
  public static int getBomLength(String aEncoding)
  {
    String wPlatformEncoding = System.getProperty("file.encoding");
    if (wPlatformEncoding != null && wPlatformEncoding.length() > 0 && wPlatformEncoding.equals(aEncoding))
      return 0;
   ...
   * 15w_003
   * 
   * The Java I/O classes will read and write Unicode data in the "default character encoding" by
   * default. The default encoding depends on some combination of the JVM, the underlying OS, and
   * the OS-level locale settings. For example, on my Windows 2000 computer, the default encoding is
   * CP-1252, a Microsoft-specific variant of Latin-1.
   * 
   * @param aBuffer
   * @return
   */
  public static String getEncoding(byte[] aBuffer)
  {
  ...
  public static byte[] hexaBufferToBytes(String aHexaBuffer) throws IllegalArgumentException
  {
    ...
    while (wI < wStreamSize)
    {
      ...
      {
        throw new IllegalArgumentException("Can't parse hexa value [" + wHexa + "] localized at offset [" + (wOffsetDot + 2) + "] in hexaBuffer [" + aHexaBuffer + "]");
      }
   ...
   * @return
   */
  public static int indexOf(byte[] data, int aOffset, int aLen, byte[] pattern)
  {
    if (aLen < 1 || aOffset < 0)
    {
      return -1;
  ...
  /**
   * 14w_009 - Int�gration WebServices
   * 
   * @param aHexaBuffer
   * @return
   * @throws IllegalArgumentException
